...
// isolation, confirming it excludes API routes, Next internals, and
// file-like paths while matching normal app routes.
const matcherRegex = new RegExp(`^${MIDDLEWARE_MATCHER}$`);

describe("middleware matcher pattern", () => {
  it.each(["/api/foo", "/api", "/_next/static/a.js", "/_vercel/insights", "/favicon.ico"])(
    "excludes %s",
    (pathname) => {
